...
router.post("/register", async (req, res) => {
  ...
  const { name, email, password, role, bloodType, location, availability } = parsed.data;
...
router.post("/login", async (req, res) => {
  ...
  if (user.role === "admin") {
    return res.status(403).json({ message: "Admin accounts must use the secret admin access page." });
  }
